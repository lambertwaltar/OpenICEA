                        ...
                        <td> 
                            <select class="form-control" name="currentartregimen">
                                ...
                                @foreach($regimens as $regimen)
                                    <option value="{{$regimen->OID}}">{{$regimen->Code}}</option>
                                @endforeach
                            </select>
                        </td>
                    </tr>
                    <tr>
                        <td>Provider</td><td> <input type="text" name="provider" value="{{Auth::user()->FirstName.' '. Auth::user()->LastName}}" readonly></td>
                    </tr>
                </table>
            </div>
            <div style="display: inline-block; min-width: 200px; margin-right:15px; vertical-align: top;width: 47%;">
                        ...
                        <td>
                            <select class="form-control" name="funding">
                                ...
                                @foreach($fundings as $funding)
                                    <option value="{{$funding->OID}}">{{$funding->Code}}:{{$funding->Name}}</option>
                                @endforeach
                            </select>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td> 
                             <select class="form-control" name="dsdmtype">
                                <option value="">---</option>
                                 @foreach($dsdms as $dsdm)
                                    <option value="{{$dsdm->OID}}">{{$dsdm->Description}}</option>
                                @endforeach
                            </select>
                        </td>
                    </tr>
                        ...
                        <td> 
                            <select class="form-control" name="schedule" id="schedule">
                                ...
                                @foreach($schedules as $schedule)
                                    <option value="{{$schedule->OID}}">{{$schedule->Frequency}} ({{$schedule->Code}})</option>
                                @endforeach
                            </select>
                        </td>
                    </tr>
...
<script type="text/javascript">
    ...
    //save prescription item
        $('#msform').on('click','#additem',function(e){
            ...
            $.ajax({
                type: 'get',
                url: '{{route("createprescriptionitem")}}',
    ...
    //delete prescription Item
        function deletepitem(r){
            ...
            $.ajax({
                type: 'get',
                url: '{{route("deleteprescriptionitem")}}',
                data:{'id':id},
                success: function(data)
                {                                           
