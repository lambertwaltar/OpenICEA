<form id="searchform" method="get" style="margin-top:10px !important; margin-left:10px !important;">
    ...
    <a href="{{ route('newflowsheet') }}" id="New Client Flowsheet" onclick="newflowsheet(this); return false;" style="margin: 6px 40px;font-size: 0.9em;"> New FlowSheet</a>
</form>
                ...
                <div class="table100-body js-pscroll" id="labtablebody">
                    <table>
                        ...
                        <tbody >
                            ...
                            <tr class="row100 head" style="height: 40px;">
                                <td class="cell100 column1">{{$flowsheet->TriageDate}}</td>
                                <td class="cell100 column5">{{$flowsheet->fspatient->returnappointment->ReturnDate.' | '.$flowsheet->fspatient->returnappointment->ReturnTime}}</td>
                                <td class="cell100 column5">@foreach($flowsheet->fsencounter->encounterreason as $reason){{$reason->Name .','}} @endforeach</td>
                                <td class="cell100 column5">
                                     <a class="editlabreq" href="{{ route('editflowsheet', ['id' => Crypt::encrypt($flowsheet->OID)]) }}" id="FlowSheet | {{$flowsheet->OID}}" onclick="editsheet(this); return false;"> View/Edit</a> | <a href="{{ route('deleteflowsheet', ['id' => Crypt::encrypt($flowsheet->OID)]) }}" onclick="deletelab(this); return false;">Delete</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
...
<script>
    ...
    //date
        $(document).ready(function () {
            ...
            //$("#labtablebody").load("{{route('flowsheets')}}"+ " #labtablebody");
