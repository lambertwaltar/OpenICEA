...
<form id="msform" class="encounterform" >
        ...
        <!-- sample information-->
            <h2 class="fs-title" style="background:#d6d4d0; padding:5px !important;font-size: 12px;">Clinical Information</h2>
             <table class="prescription_table">
                ...
                <tr>
                    <td>Collected By:</td><td> <input type="text" name="collectedby" value="{{$sample->lscprovider->FirstName.' '. $sample->lscprovider->LastName}}" readonly></td>
                </tr>
                @endforeach
                <tr>
                    <td>Collected Date:</td><td><input type="date" name="collecteddate" id="collecteddate" value="{{$result->CollectionDate}}"  readonly></td>
                    <td>Time:</td><td><input type="text" name="collectedtime" id="collectedtime" value="{{$result->CollectionTime}}" readonly></td>
                </tr>
                <tr>
                    <td>Tested By:</td><td> <input type="text" name="testedby" value="{{$result->grprovider->FirstName.' '. $result->grprovider->LastName}}" readonly></td>
                </tr>
                <tr>
                    @php
                        $time = explode(" ",$result->created_at)
                    @endphp
                    <td>Tested Date:</td><td><input type="text" name="testeddate" id="testeddate" readonly value="{{$time[0]}}"></td>
                    <td>Time:</td><td><input type="text" name="testedtime" id="testedtime" readonly value="{{$time[1]}}"></td>
                </tr>
    ...
    <div class="modal-footer">
        <a href="{{ route('printresults', ['id' => Crypt::encrypt($req->OID)]) }}" class="btn btn-secondary" id="printresults" target="_blank" style="font-size: 12px;">Print Preview</a>
