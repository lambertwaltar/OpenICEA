                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="msform" method="post" class="phoneform">
                            {{ csrf_field() }}
                            <input class="needs-validation" type="text" name="phonenumber" id="phonenumber" placeholder="Phone Number" value="{{ old('phonenumber') }}" pattern="([0-9]){10}" title="eg.0789563172" autofocus />
                            <span class="error" id="phoneerror" style="display:none;"></span> 

                            <input type="text" name="tamaphonenumber" id="tamaphonenumber" placeholder="TAMA Phone Number" value="{{ old('tamaphonenumber') }}"/>
                            @if ($errors->has('tamaphonenumber'))
                                  <span class="error">{{ $errors->first('tamaphonenumber') }}</span>
                            @endif 
                            ...
                            <input type="text" name="phonefirstname" id="phonefirstname" placeholder="First Name" value="{{ old('phonefirstname') }}"/>
                            @if ($errors->has('phonefirstname'))
                                  <span class="error">{{ $errors->first('phonefirstname') }}</span>
                            @endif

                            <input type="text" name="phonesurname" id="phonesurname" placeholder="Surname" value="{{ old('phonesurname') }}" />
                            @if ($errors->has('phonesurname'))
                                  <span class="error">{{ $errors->first('phonesurname') }}</span>
                            @endif

                            <input type="text" name="relationship" id="relationship" placeholder="Relationship" value="{{ old('relationship') }}" />
                            ...
                            <textarea name="phonedescription" id="phonedescription" placeholder="Description" value="{{ old('phonedescription') }}"></textarea>
...
<!-- Footer html -->
    <footer class="bg-white sticky-footer fixed-bottom" style="line-height: 60px; color:#ffffff; background: #999999 !important; font-family: Lato-Regular; font-size: 0.8em !important;">
		<div style="font-size:0.9em; float:left; margin:0 15px;">Current User: {{Auth::user()->FirstName.' '. Auth::user()->LastName}}</div>
...
<script type="text/javascript">
    ...
    //prescriptions form modal
        $(document).on('click', '#newprescription', function(e) {
            ...
            $.ajax({
                type: 'get',
                url: "{{route('returnappointmentsetting')}}",
                success: function(data)
                {                                           
                    for(var i=0;i<data.length;i++){
    ...
    //Prescriptions
        ...
        {       
            ...
            $.ajax({
                type: 'get',
                url: "{{route('prescriptionsforencounter')}}",
                data:{'id':id},
                success: function(data)
                {                                           
                    $('#partial').load(href,function(){
                        ...
                        for(var i = 0; i<data.length; i++){
                            ...
                            var markup = '<tr class="row100 body clickable-row"><td class="cell100 column1"><input type="hidden" name="itemid[]" value="'+data[i].OID+'">'+data[i].OID+'</td><td class="cell100 column2">'+data[i].PrescriptionDate+'</td><td class="cell100 column2">'+data[i].PrescriptionDate+'</td><td class="cell100 column4" id='+i+'></td><td class="cell100 column4"></td><td class="cell100 column2">{{Auth::user()->FirstName.' '. Auth::user()->LastName}}</td><td class="cell100 column2"><a href="{{route('editprescription',['modalroute' => 'newprescription'])}}" onclick="editprescription(this); return false;" style="cursor: pointer";><img src="images/edit.png" style="width: 20px; float: left; margin: 5px 2px;" title="Edit Prescription"></a><a href="{{route('deleteprescription')}}" onclick="deleteitem(this); return false;" style="cursor: pointer";><img src="images/remove.png" style="width: 17px; float: left; margin: 5px 2px;" title="Delete Prescription"></a></td></tr>'
        ...
        $(document).ready(function () {
            $('#partial').on('click', '#updateuser', function(e) {
                ...
                $.ajax({
                    ...
                    {
                        $('#mymodal').modal('hide');
                        window.location = "{{ route('user-management') }}";
                        console.log('success');
                    },
                    error: function(){
                        $('#mymodal').modal('hide');
                ...
                success: function(data){
                    ...
                    {   
                        for (var i = 0; i < data.length; i++){
                            ...
                            $("#newencounter").attr("href", "{{route('encounter')}}?idcno="+data[i].IDCNO+"&fname="+data[i].FirstName+"&lname="+data[i].Surname+"&status_sp="+data[i].MaritalStatus);

                            //$("#scheduleappointment").attr("href", "{{route('scheduleapointment')}}?idcno="+data[i].IDCNO+"&fname="+data[i].FirstName+"&lname="+data[i].Surname);

                            //New prescription link
                            $("#newprescription").attr("href", "");
                            $("#newprescription").attr("href", "{{route('newprescription')}}?idcno="+data[i].IDCNO+"&fname="+data[i].FirstName+"&lname="+data[i].Surname);

                            //New Lab requisition link
                            $("#newlabrequisition").attr("href", "");
                            $("#newlabrequisition").attr("href", "{{route('labrequisitionform')}}?idcno="+data[i].IDCNO+"&fname="+data[i].FirstName+"&lname="+data[i].Surname);

                            $("#viewlabs").attr("href", "");
                            $("#viewlabs").attr("href", "{{route('patientlabs')}}?idcno="+data[i].IDCNO+"&fname="+data[i].FirstName+"&lname="+data[i].Surname);

                            //Triage link
                            // $("#triageform").attr("href", "");
                            // $("#triageform").attr("href", "{{route('triageform')}}?idcno="+data[i].IDCNO+"&fname="+data[i].FirstName+"&lname="+data[i].Surname+"&dob="+data[i].BirthDate);

                            //Flowsheets
                            $("#flowsheets").attr("href", "");
                            $("#flowsheets").attr("href", "{{route('flowsheets')}}?idcno="+data[i].IDCNO+"&fname="+data[i].FirstName+"&lname="+data[i].Surname+"&dob="+data[i].BirthDate);

                            //apppintments
                            $("#scheduleappointment").attr("href", "");
                            $("#scheduleappointment").attr("href", "{{route('scheduleapointment')}}?idcno="+data[i].IDCNO+"&fname="+data[i].FirstName+"&lname="+data[i].Surname+"&dob="+data[i].BirthDate);

                            //enrolment history link
                            $("#enrolmenthistory").attr("href", "");
                            $("#enrolmenthistory").attr("href", "{{route('enrolmenthistory')}}?idcno="+data[i].IDCNO);
                            ...
                            //client actions, delete, edit
                                $('#patientactions').text('');
                                var patientdelete = '<a href="{{route('deleteclient')}}" onclick="deleteClient(this); return false;" style="cursor: pointer;" id="deleteclient"><img src="images/remove.png" style="width: 18px; margin: 5px 2px; margin-top: -1px;" title="Delete Client"> Delete Client</a>';

                                var patientedit = '<a href="{{route('clientsearch',['modalroute' => 'newclientform'])}}"  style="cursor: pointer;" id="editclient"><img src="images/edit.png" style="width: 18px; margin: 5px 2px; margin-top: -1px;" title="Edit Client" /> Edit Client</a>';
                            ...
                            for(var i = 0; i<encountersx.length; i++){
                                ...
                                var markup = "<tr class='row100 body clickable-row'><td class='cell100 column1' style='width: 8%;'><input type='hidden' name='itemid[]' value='" +encountersx[i].OID+"'>" + dateFormat(encountersx[i].visitDate) + "</td><td class='cell100 column1'>---</td><td class='cell100 column1' style='width: 8%;'>---</td><td class='cell100 column1'style='width: 5%;'>"+ isprivate[encountersx[i].IsPrivate] +"</td><td class='cell100 column1'>"+visittype[encountersx[i].VisitType]+"</td><td class='cell100 column1' id="+i+"></td><td class='cell100 column1' style='width: 5%;'>"+ visitorx[encountersx[i].Visitor] +"</td><td class='cell100 column1' style='width: 5%;'><a href='{{route("prescriptions")}}' onclick='prescriptions(this); return false;' style='cursor: pointer';><img src='images/rx.png' style='width: 15px; float: left; margin: 5px 2px;' title='Prescriptions'></a></a></td><td class='cell100 column1' style='width: 5%;'><a href='{{route("editencounterform",["modalroute" => "encounter"])}}' onclick='edititem(this); return false;' style='cursor: pointer';><img src='images/edit.png' style='width: 20px; float: left; margin: 5px 2px;' title='Edit Encounter'></a><a href='{{route("deleteencounter")}}' onclick='deleteitem(this); return false;' style='cursor: pointer';><img src='images/remove.png' style='width: 17px; float: left; margin: 5px 2px;' title='Delete Encounter'></a> </td></tr>";
    ...
    //Check Idle time and logout
        ...
        function refresh() {
            if(new Date().getTime() - time >= 3600000) 
                ...
                window.location = "{{ route('logout', ['session' => '1']) }}"; //passion url parameter
            else 
            ...
            $.ajax({
                ...
                {                                           
                    $('#partial').load(formurl+' #msform',function(){
                        ...
                        $('form').attr('action', '{{route("updateencounter")}}');
            ...
            $('#partial').on('click', '#recordencounter', function() {
                ...
                $.ajax({
                    ...
                    {   
                        $.ajax({
                            type: 'get',
                            url: '{{route("getallpatientencounters")}}',
                                        ...
                                        if(count >=1){
                                           for(var i = table.rows.length - 1; i > 0; i--){
                                            ...
                                            for(var i = 0; i<encountersx.length; i++){
                                                ...
                                                var markup = "<tr class='row100 body clickable-row'><td class='cell100 column1' style='width: 8%;'><input type='hidden' name='itemid[]' value='" +encountersx[i].OID+"'>" + dateFormat(encountersx[i].visitDate) + "</td><td class='cell100 column1'>---</td><td class='cell100 column1' style='width: 8%;'>---</td><td class='cell100 column1'style='width: 5%;'>"+ isprivate[encountersx[i].IsPrivate] +"</td><td class='cell100 column1'>"+visittype[encountersx[i].VisitType]+"</td><td class='cell100 column1' id="+i+"></td><td class='cell100 column1' style='width: 5%;'>"+ visitorx[encountersx[i].Visitor] +"</td><td class='cell100 column1' style='width: 5%;'><a href='{{route("prescriptions")}}' onclick='prescriptions(this); return false;' style='cursor: pointer';><img src='images/rx.png' style='width: 15px; float: left; margin: 5px 2px;' title='Prescriptions'></a></td><td class='cell100 column1' style='width: 5%;'><a href='{{route("editencounterform",["modalroute" => "encounter"])}}' onclick='edititem(this); return false;' style='cursor: pointer';><img src='images/edit.png' style='width: 20px; float: left; margin: 5px 2px;' title='Edit Encounter'></a><a href='{{route("deleteencounter")}}' onclick='deleteitem(this); return false;' style='cursor: pointer';><img src='images/remove.png' style='width: 17px; float: left; margin: 5px 2px;' title='Delete Encounter'></a> </td></tr>";
